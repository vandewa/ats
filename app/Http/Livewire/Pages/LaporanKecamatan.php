...
class LaporanKecamatan extends Component
{
    ...
    {
        $this->report = DB::table('ats')->select(DB::raw("DISTINCT com_regions.region_nm, coalesce(sumber, 'Pendataan Baru) as sumber, count(ats.id) as jumlah, sum(case when status = true  then 1 else 0 end ) as tervalidasi,
